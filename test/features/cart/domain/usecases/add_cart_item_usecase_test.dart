...
void main() {
  ...
  test(
    'should accumulate quantity when adding duplicate item within stock returning added',
    () async {
  ...
  test(
    'should clamp quantity to maxStock and return cappedToMax when sum exceeds stock',
    () async {
      const existingCart = CartEntity(items: [tNewItem]);
      when(() => mockRepo.getCart()).thenAnswer((_) async => existingCart);
  ...
  test(
    'should return alreadyAtMax and not save when existing item is already at max stock',
    () async {
